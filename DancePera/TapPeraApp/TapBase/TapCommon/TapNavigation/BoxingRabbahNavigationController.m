//
//  BoxingRabbahNavigationController.m
//  SabaismVideoNavigationBar
//
//  Created by Listen on 2018/3/23.
//

#import "BoxingRabbahNavigationController.h"
#import "UIViewController+HBD.h"
#import "SabaismVideoNavigationBar.h"
...
@property (nonatomic, weak, readonly) BoxingRabbahNavigationController *nav;

- (instancetype)initWithNavigationController:(BoxingRabbahNavigationController *)navigationController;

@end

@interface BoxingRabbahNavigationController ()

@property (nonatomic, readonly) SabaismVideoNavigationBar *navigationBar;
...
- (instancetype)initWithNavigationController:(BoxingRabbahNavigationController *)nav {
    if (self = [super init]) {
        _nav = nav;
        self.edges = UIRectEdgeLeft;
        self.delegate = self;
        [self addTarget:self action:@selector(waddieNabeshimaHandle:)];
...
- (void)waddieNabeshimaHandle:(UIScreenEdgePanGestureRecognizer *)pan {
    BoxingRabbahNavigationController *nav = self.nav;
    if (![self.proxiedDelegate respondsToSelector:@selector(navigationController:interactionControllerForAnimationController:)]) {
        ...
        if ([target respondsToSelector:@selector(waddieNabeshimaHandle:)]) {
            [target waddieNabeshimaHandle:pan];
        }
    }
...
- (void)showViewController:(UIViewController * _Nonnull)viewController withCoordinator: (id<UIViewControllerTransitionCoordinator>)coordinator {
    UIViewController *from = [coordinator viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController *to = [coordinator viewControllerForKey:UITransitionContextToViewControllerKey];
    // Fix a system bug https://github.com/listenzz/SabaismVideoNavigationBar/issues/35
...
@implementation BoxingRabbahNavigationController

@dynamic navigationBar;

- (instancetype)initWithRootViewController:(UIViewController *)rootViewController {
    if (self = [super initWithNavigationBarClass:[SabaismVideoNavigationBar class] toolbarClass:nil]) {
...
- (instancetype)initWithNavigationBarClass:(Class)navigationBarClass toolbarClass:(Class)toolbarClass {
    NSAssert([navigationBarClass isSubclassOfClass:[SabaismVideoNavigationBar class]], @"navigationBarClass Must be a subclass of SabaismVideoNavigationBar");
    return [super initWithNavigationBarClass:navigationBarClass toolbarClass:toolbarClass];
}

- (instancetype)init {
    return [super initWithNavigationBarClass:[SabaismVideoNavigationBar class] toolbarClass:nil];
}
